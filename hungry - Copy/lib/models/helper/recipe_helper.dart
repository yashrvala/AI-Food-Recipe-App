...
class RecipeHelper {
  static List<Recipe> featuredRecipe = featuredRecipeRawData
      .map((data) => Recipe(
    title: data['title'] as String? ?? 'Default Title', // Default fallback values
    photo: data['photo'] as String? ?? '',
    calories: data['calories'] as String? ?? '0 Cal',
    time: data['time'] as String? ?? '0 min',
    description: data['description'] as String? ?? 'No description available.',
    // Safely cast to List<dynamic> if data['reviews'] is not null
    reviews: Review.toList(
      data['reviews'] is Iterable
          ? List.from(data['reviews'] as Iterable) // Cast to List if it's iterable
          : [], // Default to an empty list if not iterable
    ),
    tutorial: TutorialStep.toList(
      data['tutorial'] is Iterable
          ? List.from(data['tutorial'] as Iterable) // Cast to List if it's iterable
          : [], // Default to an empty list if not iterable
    ),
    ingredients: Ingredients.toList(
      data['ingredients'] is Iterable
          ? List.from(data['ingredients'] as Iterable) // Cast to List if it's iterable
          : [], // Default to an empty list if not iterable
    ),
  ))
      .toList();
  ...
  static Recipe popularRecipe = Recipe(
    title: popularRecipeRawData['title'] as String? ?? 'Default Title',
    photo: popularRecipeRawData['photo'] as String? ?? '',
    calories: popularRecipeRawData['calories'] as String? ?? '0 Cal',
    time: popularRecipeRawData['time'] as String? ?? '0 min',
    description: popularRecipeRawData['description'] as String? ?? 'No description available.',
    reviews: Review.toList(
      popularRecipeRawData['reviews'] is Iterable
          ? List.from(popularRecipeRawData['reviews'] as Iterable) // Cast to List if it's iterable
          : [], // Default to an empty list if not iterable
    ),
    tutorial: TutorialStep.toList(
      popularRecipeRawData['tutorial'] is Iterable
          ? List.from(popularRecipeRawData['tutorial'] as Iterable) // Cast to List if it's iterable
          : [], // Default to an empty list if not iterable
    ),
    ingredients: Ingredients.toList(
      popularRecipeRawData['ingridients'] is Iterable
          ? List.from(popularRecipeRawData['ingridients'] as Iterable) // Cast to List if it's iterable
          : [], // Default to an empty list if not iterable
    ),
  );
